...
void mvoxAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    // 3. De-esser (spectral reduction using filtered sidechain)
    {
        juce::AudioBlock<float> oversampledBlockCopy = block;
        juce::dsp::ProcessContextReplacing<float> filteredCtx(oversampledBlockCopy);
        deesserFilter.process(filteredCtx);
        
        float deessAmount = *apvts.getRawParameterValue(deesserAmountId) / 100.0f;
        for (size_t ch = 0; ch < block.getNumChannels(); ++ch)
        {
            auto* channelData = block.getChannelPointer(ch);
            auto* filteredData = oversampledBlockCopy.getChannelPointer(ch);
    ...
    // 4. Saturation (manual tanh waveshaping)
    {
