...
private:
    ...
    // Simple de-esser (sidechain compressor)
    juce::dsp::Compressor<float> deesser;
    juce::dsp::IIR::Filter<float> deesserFilter;
    
    // Simple saturation using waveshaper
    juce::dsp::ProcessorDuplicator<juce::dsp::WaveShaper<float>, juce::dsp::WaveShaper<float>> saturator;
    
    // Lookahead limiter
    juce::dsp::Limiter<float> limiter;

    // Air EQ boost shelving filter
